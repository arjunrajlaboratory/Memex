...
const { isSafeExternalUrl, isTrustedFileUrl, resolveInside, resolvedStaysInside } = require('../dist/main/security.js');
...
test('resolved vault paths reject relative values and symlink escapes', (t) => {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), 'memex-security-'));
  t.after(() => fs.rmSync(root, { recursive: true, force: true }));
  const vault = path.join(root, 'vault');
  const outside = path.join(root, 'outside');
  fs.mkdirSync(vault);
  fs.mkdirSync(outside);
  fs.symlinkSync(outside, path.join(vault, 'escape'), process.platform === 'win32' ? 'junction' : 'dir');

  const futureFile = path.join(vault, 'new', 'report.md');
  assert.equal(resolvedStaysInside(vault, futureFile), true);
  assert.equal(resolvedStaysInside(vault, 'new/report.md'), false);
  assert.equal(resolvedStaysInside(vault, path.join(vault, 'escape', 'secret.md')), false);
  assert.equal(resolvedStaysInside(vault, path.join(outside, 'secret.md')), false);
});
