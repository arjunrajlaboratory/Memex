...
20. **Title ↔ filename drift.** Two parts (per `_schemas/_types.md` → "Filenames and titles"). **(a)** Every typed note's filename stem must be `safe_title`-clean — flag any filename containing `/ : \ * ? " < > | # ^ [ ]`. This is the **upstream cause** of most check-#1 broken wikilinks: when a hazardous char is dropped/altered on save, every `[[name]]` written elsewhere points at the raw form, not the file. **(b)** For notes whose filename is *derived from a frontmatter field* — Task/Idea (`title:`), Organization/Person (`name:`) — the filename stem must also equal `safe_title` of that field; flag drift. **Exemptions from (b):** date/id-derived filenames (journals, briefings, reviews, agent jobs/runs); and **Grant**, whose `title:` is the full proposal title and is intentionally distinct from its short filename (apply only part (a) to Grants — never flag Grant `title:` ≠ filename as drift). Most other types (Source, Decision, Project, Concept, Tracker, Effort, Area, Implementation) have no separate title field, so only part (a) applies. Lean high-severity. For each hit, report the filename, the offending field (if any), and the `safe_title` form the file should use.
