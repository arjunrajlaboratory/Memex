...
File path: `Atlas/Sources/<Display Title>.md` — run the title through `safe_title` first (see `_schemas/_types.md` → "Filenames and titles"; article titles carry `:`/`/` constantly): filename stem = `title:` = every `[[wikilink]]`; no `/ : \ * ? " < > | # ^ [ ]`.
