...
1. **Subject** — One short phrase. This becomes `Atlas/Projects/<Subject>.md` (the filename is the bare subject per `_schemas/project.md` — no `Project - ` prefix). Run it (and every entity name you mint in this run) through `safe_title` first (see `CLAUDE.md`/`AGENTS.md` → "A note's title IS its filename"): ` / ` → ` and ` (bare `/` → `-`), drop `:` and the rest of `\ * ? " < > | # ^ [ ]`, collapse spaces — so the filename stem and every `[[wikilink]]` to it stay identical.
